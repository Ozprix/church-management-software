    ...
    <div v-else class="bg-white rounded-lg shadow-md p-6">
      <form @submit.prevent="updateExpense">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <!-- Current Receipt -->
          <div v-if="expense.receipt_url" class="md:col-span-2">
...
export default {
  ...
  methods: {
    async fetchExpense() {
      ...
      try {
        const response = await axios.get(`/expenses/${this.expenseId}`);
        ...
        if (response.data.status === 'success') {
          ...
          if (expenseData.expense_date) {
            expenseData.expense_date = this.formatDateForInput(expenseData.expense_date);
          }
        ...
        this.error = error.response?.data?.message || 'An error occurred while fetching the expense';
      } finally {
        this.loading = false;
      }
    },
    ...
    async updateExpense() {
      ...
      try {
        ...
        Object.keys(this.expense).forEach(key => {
          // Skip receipt_url as it's not needed for update
          if (key !== 'receipt_url') {
        ...
        const response = await axios.post(`/expenses/${this.expenseId}?_method=PUT`, formData, {
          headers: {
        ...
        if (error.response?.status === 422) {
          ...
          this.error = error.response?.data?.message || 'An error occurred while updating the expense.';
        }
      } finally {
        this.saving = false;
      }
