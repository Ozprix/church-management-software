<template>
  <div class="container mx-auto px-4 py-8">
    <div class="flex items-center mb-6">
      ...
      <h1 class="text-2xl font-bold text-gray-800">Record New Donation</h1>
    </div>
...
export default {
  ...
  data() {
    return {
      donation: {
        ...
        transaction_id: '',
        campaign_id: '',
        donation_date: new Date().toISOString().slice(0, 10), // Today's date
        is_recurring: false,
  ...
  methods: {
    ...
    async saveDonation() {
      ...
      try {
        const response = await axios.post('/donations', this.donation);
        ...
        if (error.response?.status === 422) {
          ...
          this.error = error.response?.data?.message || 'An error occurred while saving the donation.';
        }
      } finally {
        this.saving = false;
      }
