<template>
  <div class="member-tax-receipts">
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
      ...
      <div v-else-if="receipts.length === 0" class="p-6 text-center">
        <div class="text-gray-400 mb-2">
          <i class="fas fa-file-invoice-dollar text-4xl"></i>
        </div>
        <p class="text-gray-600">No tax receipts found for this member</p>
        <p class="text-sm text-gray-500 mt-1">Generate a tax receipt for a donation or create an annual receipt</p>
      </div>
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div class="mt-2">
                  <p class="text-sm text-gray-500">
                    Generate an annual tax receipt for all donations in the selected year.
                  </p>
                  <div class="mt-4">
                  ...
                  <div v-if="yearSummary" class="mt-4 bg-gray-50 p-3 rounded-md">
                    ...
                    <div v-if="yearSummary.has_receipt" class="mt-2 p-2 bg-yellow-50 text-yellow-700 text-xs rounded">
                      <i class="fas fa-exclamation-triangle mr-1"></i> This member already has an annual receipt for this year. Generating a new one will void the previous receipt.
                    </div>
                    <div v-if="yearSummary.donation_count === 0" class="mt-2 p-2 bg-red-50 text-red-700 text-xs rounded">
                      <i class="fas fa-exclamation-circle mr-1"></i> This member doesn't have any donations for this year.
                    </div>
                  </div>
                </div>
              </div>
...
export default {
  ...
  setup(props) {
    ...
    const fetchReceipts = async () => {
      loading.value = true;
      try {
        const response = await axios.get(`/api/members/${props.memberId}/tax-receipts`);
    ...
    const generateAnnualReceipt = async () => {
      ...
      try {
        ...
        toast.success(`Annual tax receipt for ${annualReceiptYear.value} generated successfully`);
